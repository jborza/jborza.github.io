...
I thought that making multiple games in one is more fun than just a single one, so I've built four: Snake, Brick Shooter, Breakout, Cars. 

When you start the game, a little menu appears showing the selected game and difficulty.
...
It's written in C# targeting the .NET Framework, rendering courtesy of the GDI+ graphics library. 

The tray icon has a canvas of 16x16 pixels, 

Because we don't have meaningful way to give focus to a tray icon, we hijack the global keyboard events using the [RegisterHotKey](https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-registerhotkey) API.

### Why Code Golfing?

After the game was completed it was done, the code looked like a good candidate for golfing - or making the code as short as possible while still retaining the functionality.
